...
export default function Dashboard() {
  const { user } = useAuth();
  const { data: stats, isLoading } = useQuery({ queryKey: ["stats"], queryFn: async () => (await api.get("/dashboard/stats")).data });
  const { data: activities = [] } = useQuery({ queryKey: ["activities"], queryFn: async () => (await api.get("/activities?limit=8")).data });
  const { data: followups = [] } = useQuery({ queryKey: ["followups", "today"], queryFn: async () => (await api.get("/followups?scope=today")).data });
