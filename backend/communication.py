...
class ExotelAdapter:
    ...
    async def initiate_call(self, from_number, to_number, custom_field=None):
        payload = {
            "from": {"number": from_number} if from_number else {"user_contact_uri": "sip:agent@exotel.com"},
            "to": {"number": to_number},
            "virtual_number": self.virtual,
            "recording": True, "recording_channels": "dual",
            "custom_field": custom_field,
        }
        if self.callback:
            payload["status_callback"] = [{"event": "terminal", "url": self.callback + "/api/webhooks/exotel/call"}]
        async with httpx.AsyncClient(timeout=20) as c:
            r = await c.post(f"{self.voice_base}/v2/accounts/{self.sid}/calls", auth=(self.key, self.token), json=payload)
            if r.status_code >= 400:
                raise CommunicationError(f"Exotel call error {r.status_code}")
            data = r.json()
            call = data.get("response", data.get("Call", data))
            cid = call.get("call_sid") or call.get("Sid")
            return {"provider_call_id": cid,
                    "status": map_call_status(call.get("call_state", call.get("Status", "initiated"))),
                    "raw": data}
